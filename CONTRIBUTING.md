...
Playwright is a multi-package repository that uses pnpm workspaces. For browser APIs, look at [`packages/playwright-core`](https://github.com/microsoft/playwright/blob/main/packages/playwright-core). For test runner, see [`packages/playwright`](https://github.com/microsoft/playwright/blob/main/packages/playwright).
...
Various other files are generated from the API specification. If you are running `pnpm watch`, these will be re-generated automatically.
...
```bash
CRPATH=<path-to-executable> pnpm ctest
```
