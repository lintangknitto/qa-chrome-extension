...
Do not assume a package manager or treat the nearest manifest as the install root. Apply this order:
...
- **Never apply forced audit remediation automatically** (`ppnpm audit --fix --force` or equivalent). Preview the remediation, read changelogs, and test each resulting upgrade; forced fixes may cross declared dependency ranges.
- **Verify registry signatures and provenance where supported** (`ppnpm audit`, `pppnpm audit`) and treat absence as a signal to investigate, not automatic proof of compromise.
