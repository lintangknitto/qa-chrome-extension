...
- [x] Jalankan `/promote` setelah `/qa` dan `/gate` dinyatakan lolos. (PR chrome **dilewati** sesuai keputusan user — topologi `qa-chrome-extension` divergen: `origin/main`=`6eed5f291` (lineage `knitto-main`) vs `main` lokal (6 komit termasuk upstream roll); FAB juga bergantung pada komit recorder/init. Perlu keputusan rebase tersendiri bila mau di-PR. Backend tidak berubah — tidak ada PR API.)