...
- [x] Jalankan `/promote` setelah `/qa` dan `/gate` dinyatakan lolos. (refine 3.3 — PR chrome **dilewati** sesuai keputusan user berulang (topologi `qa-chrome-extension` divergen); dist extension sudah rebuild fresh; backend tidak berubah → tanpa PR API.) *(Stage SHIP 2026-09-22: branching model `releases/*` tidak berlaku (Step 0: tidak ada releases/*, -main/-dev, atau CI workflow); user mengonfirmasi ulang **skip PR dan tandai shipped** — tidak ada push/PR ke lintangknitto/qa-chrome-extension atau microsoft/playwright; plan di-archive di `docs/prd/done/extension-floating-button/`)*