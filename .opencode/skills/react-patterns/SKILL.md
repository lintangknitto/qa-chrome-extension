...
> **Project note (Knitto Multichannel Chat):** this skill covers component *logic* patterns
> (hooks, state, composition, data fetching) generically. For where files/folders should
> physically live under `apps/dashboard/src/pages/**` and `apps/widget/src/pages/**`
> specifically, that's governed by `CLAUDE.md` § "Frontend component structure" instead —
> a Knitto-production-derived rule (flat file vs. `{section-name}/` folder vs. `hooks/`/
> `icons/`/`context/`/`utils/` subfolders), which takes precedence over any generic
> file-layout assumption in the sections below.
