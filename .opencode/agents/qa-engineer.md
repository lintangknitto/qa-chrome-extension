...
description: >-
  ...
  ini", or when a feature/PRD is about to ship without test coverage.
  Always starts by writing a test-case-matrix (scenario list) before
...
- Installing `webapp-testing`'s local pre-push git hook — that's opt-in;
  don't install it unless explicitly asked, since it changes the behavior
  of every future `git push` on the machine.
...
copy of this agent at `.opencode/agents/qa-engineer.md` (or wherever this
repo's agents live) that states those specifics explicitly.
